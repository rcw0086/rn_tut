...
fn main() {
    ...
    println!("My original arg is {}", arg);
    inspect(&arg); // -> note, Rust creates a reference here, with my call
    change(&mut arg); // -> note, Rust creates a mutable reference here, with my call
    println!("My mutated arg is {}", arg); // MUTATION has happened here; prints change() output
    ...
    fn eat(s: String) -> bool {
        return if s.starts_with("b") && s.ends_with("s") {
            true
        } else {
            false
        };
